...
import { WorkspaceWithDetails } from '@/app/workspaces/page';
...
const Sidebar = () => {
    ...
    const { data: memberships, isLoading } = useQuery<WorkspaceMember & {workspace: WorkspaceWithDetails}[]>({
        queryKey: ['workspaces'],
    ...
    useEffect(() => {
        ...
        if (!activeWorkspace && memberships && memberships.length > 0) {
            ...
            setActiveWorkspace({
                ...
                // در کد قبلی موارد زیر بود
                // id: firstMembership.workspaceId,
                // name: firstMembership.workspace.name,
                // role: firstMembership.role
                  id: firstMembership.workspace.workspaceId,
                name: firstMembership.workspace.workspace.name,
                role: firstMembership.workspace.role
            });
        }
            ...
            <Flex direction="column" className="p-3 flex-1 h-full">
                ...
                <DropdownMenu.Root>
                    ...
                    <DropdownMenu.Content align="start" side="top">
                        ...
                        <DropdownMenu.Separator/>
                        <DropdownMenu.Item onSelect={() => signOut()} color="red"><ExitIcon/> خروج</DropdownMenu.Item>
                    </DropdownMenu.Content>
                </DropdownMenu.Root>
            </Flex>
        </aside>

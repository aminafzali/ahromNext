// File: app/components/sidebar/Sidebar.tsx (نسخه نهایی با اصلاح خروج)
'use client';
...
import { Avatar, Box, Button, DropdownMenu, Flex, Heading, IconButton, Separator, Text, Tooltip } from '@radix-ui/themes';
import { ExitIcon, PlusCircledIcon, ShuffleIcon, DoubleArrowRightIcon, DoubleArrowLeftIcon } from '@radix-ui/react-icons';
...
const Sidebar = () => {
    ...
    const { data: memberships, isLoading } = useQuery<any[]>({ // 'any' برای سادگی در اینجا
        queryKey: ['workspaces'],
    ...
    useEffect(() => {
        ...
        if (!activeWorkspace && memberships && memberships.length > 0) {
            ...
            if (firstMembership && firstMembership.workspace) {
                setActiveWorkspace({
        ...
        <aside className={classNames(
            ...
            <Flex direction="column" className="p-3 flex-1 h-full">
                ...
                <Flex align="center" justify="between" className="px-1" mb="4">
                    {!isCollapsed && <Link href="/dashboard"><Heading>اهرم</Heading></Link>}
                    ...
                    <DropdownMenu.Trigger>
                        <Button variant="outline" className="w-full justify-between">
                            <Flex align="center" gap="2" className="truncate">
                                <Avatar fallback={activeWorkspace?.name?.charAt(0) || 'W'} size="1" radius='full'/>
                                {!isCollapsed && <Text className="truncate">{activeWorkspace?.name || "انتخاب ورک‌اسپیس"}</Text>}
                            </Flex>
                        </Button>
                    </DropdownMenu.Trigger>
                    <DropdownMenu.Content align="start" className="w-[250px]">
                        {memberships?.map((membership: any) => (
                           <DropdownMenu.Item key={membership.workspaceId} onSelect={() => handleWorkspaceChange(membership)}>
                                {membership.workspace.name}
                           </DropdownMenu.Item>
                        ))}
                        <DropdownMenu.Separator />
                        <DropdownMenu.Item onSelect={() => router.push('/dashboard')}><ShuffleIcon /> مدیریت ورک‌اسپیس‌ها</DropdownMenu.Item>
                        <DropdownMenu.Item onSelect={() => router.push('/workspaces/new')}><PlusCircledIcon /> ایجاد ورک‌اسپیس جدید</DropdownMenu.Item>
                    </DropdownMenu.Content>
                </DropdownMenu.Root>
                
                <Separator my="4" size="4" />
                <Box className="flex-grow overflow-y-auto"><SidebarNavLinks isCollapsed={isCollapsed} /></Box>
                ...
                <DropdownMenu.Root>
                    <DropdownMenu.Trigger>
                        <button className="w-full p-2 rounded-md hover:bg-gray-100 dark:hover:bg-gray-800">
                            <Flex gap="3" align="center">
                                <Avatar src={session?.user?.image!} fallback={session?.user?.name?.charAt(0) || '?'} size="2" radius='full' />
                                {!isCollapsed && <Text weight="bold" size="2" className="truncate">{session?.user?.name}</Text>}
                            </Flex>
                        </button>
                    </DropdownMenu.Trigger>
                    <DropdownMenu.Content align="start" side="top">
                        ...
                        <DropdownMenu.Item onSelect={() => signOut({ callbackUrl: '/' })} color="red">
                            <ExitIcon/> خروج
                        </DropdownMenu.Item>
                    </DropdownMenu.Content>
                </DropdownMenu.Root>

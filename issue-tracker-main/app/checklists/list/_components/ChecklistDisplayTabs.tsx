...
import { Flex, Grid, Heading, Text, Card, Box, Button as RadixButton, Tabs, Table, Badge, Select, Tooltip } from "@radix-ui/themes";
...
import { ChecklistTemplate, ResponseStatus, Category, Tag, ChecklistAssignment, CategoryOnChecklistTemplates, TagOnChecklistTemplates } from "@prisma/client";
import { ExtendedChecklistAssignment, ChecklistAssignmentQuery, TemplateWithDetails, ResponseFilterStatus } from "../page";
import { useRouter, useSearchParams } from "next/navigation";
import CategoryTreeView from './CategoryTreeView';
import ChecklistSettings from "./ChecklistSettings";

// ========== پراپ‌های کامپوننت اصلی ==========
interface ChecklistDisplayTabsProps {
  templates: (ChecklistTemplate & { _count: { items: number }, categories: (CategoryOnChecklistTemplates & {category: Category})[], tags: (TagOnChecklistTemplates & {tag: Tag})[] })[];
  allCategories: Category[];
  allTags: Pick<Tag, 'id' | 'name' | 'color'>[];
  assignments: (ChecklistAssignment & { template: ChecklistTemplate & {_count: {items: number}, categories: (CategoryOnChecklistTemplates & {category: Category})[], tags: (TagOnChecklistTemplates & {tag: Tag})[]}, responses: {status: ResponseStatus}[], assignedToUser: {id:string, name: string|null, email:string|null} | null})[];
...
// کامپوننت کارت الگو
const ChecklistTemplateCard = ({ template }: { template: ChecklistDisplayTabsProps['templates'][0] }) => {
  return (
    <Card variant="classic" className={`hover:shadow-lg transition-shadow duration-200 dir-rtl border border-gray-200 dark:border-gray-700 rounded-lg ${!template.isActive && 'opacity-60 bg-gray-50 dark:bg-gray-800/50'}`}>
        ...
        <Box>
          <Flex justify="between" align="start">
            <Heading as="h3" size="4" mb="1" trim="start" className="text-right">
              <Link href={`/checklists/templates/${template.id}`} className="hover:text-blue-600 dark:hover:text-blue-400">{template.title}</Link>
            </Heading>
          ...
          <Flex direction="column" gap="2" mt="2" align="end">
            {template.categories.length > 0 && (
              <Flex wrap="wrap" gap="1" justify="end">
                  <Tooltip content="دسته‌بندی‌ها"><ReaderIcon className="w-3.5 h-3.5 text-gray-500 mt-px" /></Tooltip>
                {template.categories.map(c => (<Badge key={c.categoryId} color="purple" variant="soft" radius="full" size="1">{c.category.name}</Badge>))}
              </Flex>
            )}
            {template.tags.length > 0 && (
              <Flex wrap="wrap" gap="1" justify="end">
                  <Tooltip content="برچسب‌ها"><BookmarkIcon className="w-3.5 h-3.5 text-gray-500 mt-px" /></Tooltip>
                {template.tags.map(t => (<Badge key={t.tagId} color={t.tag.color as any} variant="soft" radius="full" size="1">{t.tag.name}</Badge>))}
              </Flex>
            )}
          </Flex>
        </Box>
        <Flex mt="3" justify="start" align="center" gap="2">
          <RadixButton asChild variant="soft" size="1"><Link href={`/checklists/templates/${template.id}`}><EyeOpenIcon className="ml-1 rtl:mr-1 rtl:ml-0" /> مشاهده و تخصیص</Link></RadixButton>
           <Text size="1" color="gray" className="self-center mr-auto">{template._count.items} آیتم</Text>
        </Flex>
      </Flex>
    </Card>
  );
...
const ChecklistAssignmentsTable = ({ assignments, searchParams, currentUserId }: { assignments: ChecklistDisplayTabsProps['assignments'], searchParams: ChecklistAssignmentQuery, currentUserId: string | undefined }) => {
    const getAssignmentStatus = (assignment: ChecklistDisplayTabsProps['assignments'][0]): { text: string; color: "gray" | "blue" | "green" | "red" } => {
    ...
    const assignmentTableColumns: { label: string; value?: ChecklistAssignmentQuery['orderBy'] | 'dueDate'; className?: string; isSortable?: boolean; }[] = [
              ...
              return (
                <Table.Row key={assignment.id} className={`hover:bg-gray-100 dark:hover:bg-gray-700 ${index % 2 === 0 ? 'bg-white dark:bg-gray-900' : 'bg-gray-50 dark:bg-gray-800/50'}`}>
                  <Table.Cell className="text-right py-3 px-4">
                    <Link href={`/checklists/templates/${assignment.template.id}`} className="hover:text-blue-600 dark:hover:text-blue-400 font-medium">{assignment.template.title}</Link>
                     <Box className="md:hidden mt-1 text-right">
                        {templateCategories.length > 0 && templateCategories.slice(0,1).map(c => <Badge key={c.categoryId} color="purple" variant="soft" radius="full" size="1" mr="1">{c.category.name}</Badge>)}
                        {templateTags.length > 0 && templateTags.slice(0,1).map(t => <Badge key={t.tagId} color={t.tag.color as any} variant="soft" radius="full" size="1">{t.tag.name}</Badge>)}
                     </Box>
                  ...
                  <Table.Cell className="hidden lg:table-cell text-right py-3 px-4">
                    <Flex direction="column" gap="1">
                      {templateCategories.map(c => <Badge key={c.categoryId} color="purple" variant="soft">{c.category.name}</Badge>)}
                      {templateTags.map(t => <Badge key={t.tagId} color={t.tag.color as any} variant="soft">{t.tag.name}</Badge>)}
...
// کامپوننت فیلترهای تب الگوها
const TemplateFilters = ({ allCategories, allTags, searchParams }: { allCategories: Category[], allTags: Pick<Tag, 'id' | 'name' | 'color'>[], searchParams: ChecklistAssignmentQuery }) => {
    ...
    return (
        <Flex gap="3" wrap="wrap" justify="start" className="dir-rtl">
            <Select.Root value={searchParams.category || 'all'} onValueChange={(value) => handleFilterChange('category', value)}>
                <Select.Trigger radius="full" placeholder="دسته‌بندی..." />
                <Select.Content>
                    <Select.Item value="all">همه دسته‌بندی‌ها</Select.Item>
                    {allCategories.map(cat => ( <Select.Item key={cat.id} value={cat.name}>{cat.name}</Select.Item> ))}
                </Select.Content>
            </Select.Root>
            <Select.Root value={searchParams.tag || 'all'} onValueChange={(value) => handleFilterChange('tag', value)}>
                <Select.Trigger radius="full" placeholder="برچسب..." />
                <Select.Content>
                    <Select.Item value="all">همه برچسب‌ها</Select.Item>
                    {allTags.map(tag => ( <Select.Item key={tag.id} value={tag.name}>{tag.name}</Select.Item> ))}
                </Select.Content>
            </Select.Root>
             <Select.Root value={searchParams.templateStatus || 'active'} onValueChange={(value) => handleFilterChange('templateStatus', value as 'active' | 'archived' | 'all')}>
...
const ChecklistDisplayTabs: React.FC<ChecklistDisplayTabsProps> = ({
  ...
  searchParams, currentUserId, pageSize, currentPage, defaultTab,
}) => {
  const router = useRouter();
  const currentQueryParams = useSearchParams();
  ...
  return (
    <Tabs.Root defaultValue={defaultTab} onValueChange={handleTabChange} dir="rtl">
      <Tabs.List size="2" className="border-b border-gray-300 dark:border-gray-700">
        <Tabs.Trigger value="templates"><Flex align="center" gap="2"><FileTextIcon /> قالب‌های چک‌لیست</Flex></Tabs.Trigger>
        <Tabs.Trigger value="assignments"><Flex align="center" gap="2"><ListBulletIcon /> چک‌لیست‌های اختصاص داده شده</Flex></Tabs.Trigger>
        <Tabs.Trigger value="settings"><Flex align="center" gap="2"><GearIcon /> تنظیمات</Flex></Tabs.Trigger>
      </Tabs.List>
          ...
          <Grid columns={{ initial: '1', md: '280px 1fr' }} gap="6" className="dir-rtl">
            ...
            <main>
                <Flex direction="column" gap="4">
                    <Flex direction={{initial: "column-reverse", sm: "row"}} justify="between" align={{initial: "stretch", sm: "center"}} gap="3" mb="4">
                        <TemplateFilters allCategories={allCategories} allTags={allTags} searchParams={searchParams} />
                         <RadixButton asChild className="w-full sm:w-auto shrink-0"><Link href="/checklists/new"><PlusIcon /> اضافه کردن قالب</Link></RadixButton>
                    </Flex>
                    {templates.length === 0 && (<Text color="gray" className="text-center py-10 text-lg">موردی یافت نشد.</Text>)}
